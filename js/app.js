...
function withdrawCard(playerName) {
  if (firstWithdraw) {
    ...
    if (playerName.name.text() === userNameHtml.text()) {
      ...
      playerName.cardsArr.push(newCard > 13 ? 13 : newCard);
    } else {
  ...
  if (sameSum === 2) {
    ...
    for (let i = 0; i < playersArray.length; i++) {
      if (playersArray[i].sum !== highestSum) {
        ...
        playersArray[i].chipsHtml.html(
          `Chips = $ ${playersArray[i].chips} ${loseImg}`
        );
      }
    }
    for (let i = 0; i < playersArray.length; i++) {
      if (playersArray[i].sum === highestSum) {
        ...
        playersArray[i].chipsHtml.html(
          `Chips = $ ${playersArray[i].chips} ${winImg}`
        );
      }
    }
  } else {
